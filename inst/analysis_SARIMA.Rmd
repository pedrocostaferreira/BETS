...
info[1,] <- c(code,params$description,frequency(data),start(data),"Custom",params$unit)
```
...
preds = BETS.predict(model,h=n.ahead, main = info[,"Description"], ylab = info[,"Unit"], knit = T)
data = c(data,preds)
...
if(grepl("\\.spss$", series.file)){
  ...
  write.csv(data, file = series.file)
} else if(grepl("\\.csv2$", series.file)) {
  write.csv2(data, file = series.file)
}
```
