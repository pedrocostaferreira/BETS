...
## User-Defined Parameters
 ...
 ------------------ | ------------------ | ----------
 Series Code        | `r as.integer(params$ts)`    | `ts` 
...
```{r echo = 1}
BETS.corrgram(ts, lag.max = lag.max, mode = "bartlett", knit = T)
```

### PACF - Original Series

```{r}
BETS.corrgram(ts, lag.max = lag.max, mode = "simple", type = "partial", knit = T)
```
